...
// ─── HERO PAGE ────────────────────────────────────────────────────────────────
function HeroPage({ onSelectRole }) {
...
function LoginPage({ role, onBack, onLogin }) {
  ...
  const roleColor = role === "doctor" ? T.vital : T.rose;
  const roleAccent = role === "doctor" ? "#3A8A6A" : T.roseDeep;
  ...
  return (
    <div style={{ position:"fixed", inset:0, display:"flex", overflow:"hidden" }}>
      ...
      {/* Left brand panel */}
      <div style={{ width:"44%", position:"relative", overflow:"hidden", background:`linear-gradient(160deg,${roleAccent} 0%,${role === "doctor" ? "#1A5A3A" : "#7A2525"} 100%)`, display:"flex", flexDirection:"column", justifyContent:"center", padding:"56px 48px" }}>
        ...
        <div style={{ animation:"slideR .6s ease" }}>
          ...
          <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:800, fontSize:42, color:T.white, letterSpacing:"-0.04em", lineHeight:1.05, marginBottom:10 }}>
            {roleLabel}<br/>Portal
          </div>
          <div style={{ fontFamily:"'Playfair Display',serif", fontStyle:"italic", fontSize:16, color:"rgba(255,255,255,.65)", lineHeight:1.65, marginBottom:44 }}>
            {role === "doctor"
              ? "Clinical intelligence and patient management at your fingertips."
              : "Your personal health dashboard, powered by AI."}
          </div>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
          {(role === "doctor"
            ? [[<Icons.stethoscope/>,"Clinical queue & patient management"],[<Icons.heartbeat/>,"Presage AI critical alerts"],[<Icons.shield/>,"Secure Auth0 authentication"],[<Icons.mapPin/>,"Department capacity overview"]]
            : [[<Icons.shield/>,"Auth0 secured authentication"],[<Icons.card/>,"Instant health card scanning"],[<Icons.heartbeat/>,"AI-powered triage with Presage"],[<Icons.mapPin/>,"Real-time hospital routing"]]
          ).map(([ic,text],i)=>(
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"center", padding:"44px 52px", background:T.bg, overflowY:"auto" }}>
        <div style={{ width:"100%", maxWidth:400, animation:"fadeUp .5s ease" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
            <div style={{ width:28, height:28, borderRadius:8, background:`${roleColor}15`, border:`1px solid ${roleColor}30`, display:"flex", alignItems:"center", justifyContent:"center", color:roleColor }}>
              {role === "doctor" ? <Icons.stethoscope/> : <Icons.user/>}
            </div>
            <span style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:roleColor, letterSpacing:"0.14em", textTransform:"uppercase" }}>{roleLabel} Sign In</span>
          </div>
          <div style={{ fontFamily:"'Outfit',sans-serif", fontWeight:800, fontSize:28, color:T.ink, letterSpacing:"-0.03em", marginBottom:4 }}>Welcome back</div>
          <div style={{ fontFamily:"'Outfit',sans-serif", fontSize:14, color:T.inkFaint, marginBottom:26 }}>Access your Sanctii {roleLabel.toLowerCase()} dashboard</div>

          {/* Mode tabs */}
          <div style={{ display:"flex", marginBottom:20, borderBottom:`2px solid ${T.bgDeep}` }}>
            {(role === "patient"
              ? [["card","Health Card"],["manual","Email & Password"]]
              : [["manual","Email & Password"]]
            ).map(([m,label])=>(
              <button key={m} onClick={()=>setMode(m)} style={{ flex:1, padding:"10px 0", border:"none", background:"transparent", cursor:"pointer", fontFamily:"'Outfit',sans-serif", fontWeight:500, fontSize:12, color:mode===m?roleColor:T.inkFaint, borderBottom:`2px solid ${mode===m?roleColor:"transparent"}`, marginBottom:-2, transition:"all .2s" }}>{label}</button>
            ))}
          </div>

          {/* Health Card scan (patients only) */}
          {mode==="card" && role==="patient" && (
          ...
          {mode==="manual"&&(
            <div style={{ animation:"fadeUp .3s ease", display:"flex", flexDirection:"column", gap:13 }}>
              {[["Email address","email",email,setEmail,"you@example.com"],["Password","password",password,setPassword,"••••••••"]].map(([label,type,val,setter,ph])=>(
                <div key={label}>
                  ...
                  <input type={type} value={val} onChange={e=>setter(e.target.value)} placeholder={ph}
                    ...
                    onFocus={e=>{e.target.style.borderColor=roleColor; e.target.style.background=T.surfaceHard;}}
                    onBlur={e=>{e.target.style.borderColor=T.border; e.target.style.background=T.bgDeep;}}
                  />
                </div>
              ))}
              <button className="btn-primary" onClick={()=>onLogin(role)}
                style={{ width:"100%", padding:"13px 0", fontSize:14, marginTop:4, background:`linear-gradient(135deg,${roleColor},${roleAccent})` }}>
              ...
              <button onClick={()=>onLogin(role)}
                style={{ width:"100%", padding:"12px 0", borderRadius:10, border:`1.5px solid ${T.border}`, background:T.surfaceHard, cursor:"pointer", fontFamily:"'Outfit',sans-serif", fontWeight:500, fontSize:13, color:T.ink, display:"flex", alignItems:"center", justifyContent:"center", gap:10, transition:"all .2s" }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor=roleColor;}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor=T.border;}}>
                <div style={{ width:20, height:20, borderRadius:"50%", background:roleColor, display:"flex", alignItems:"center", justifyContent:"center", color:T.white, fontSize:11, fontWeight:700 }}>A</div>
                Continue with Auth0
              </button>
            </div>
          )}
...
export default function App() {
  const [view, setView] = useState("hero"); // "hero" | "login" | "app"
  ...
  if (view === "app") {
    return (
      <>
        <GlobalStyle/>
        <Landing role={role} onLogout={handleLogout}/>
      </>
    );
  }
  ...
  return (
    <div style={{ width:"100vw", height:"100vh", overflow:"hidden", position:"relative" }}>
      ...
      {view === "hero" && <HeroPage onSelectRole={handleSelectRole}/>}
      {view === "login" && <LoginPage role={role} onBack={() => setView("hero")} onLogin={handleLogin}/>}
    </div>
  );
}
